...
FROM PedidoApoio INNER JOIN Pessoa P ON P.id = pedinte CROSS JOIN Abrigo A2

WHERE tipo LIKE 'Alojamento' AND PedidoApoio.id NOT IN (
    SELECT pedido
    FROM ApoioAlojamento INNER JOIN Apoio A ON A.id = ApoioAlojamento.id
)
GROUP BY pedinte;
